...
# (the plugin ABI is tied to the exact Hyprland commit). Restart Hyprland to pick
# up a rebuilt .so — do not hot-reload via hyprctl (unload crashes the compositor).
#
...
build_plugin() {
    ...
    echo "  Restart Hyprland to load the updated plugin (hot-reload is not supported)."
}
...
case "${1:-install}" in
    --uninstall)
        ...
        if [[ -f "$DEST/plugins/hyprdesktop.so" ]]; then
            ...
            echo "  removed hyprdesktop.so (restart Hyprland to unload from memory)"
        fi
